...
%FILTFILT Zero-phase forward and reverse digital filtering. (This version
%is identical to the original filtfilt, but is faster for multiple columns).
...
% This reduces end effects.
    y = [bsxfun(@minus, 2*x(1,:), x((nfact+1):-1:2,:));
         x;
         bsxfun(@minus, 2*x(len,:), x((len-1):-1:len-nfact,:))];
