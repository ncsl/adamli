...
%      raw files (reref and noreref) for stimulation.  This functionality could be 
%      incorporated into EEGprepAndAlign (this file is basically a copy of the first part of EEGprepAndAlign)
%      For now keep it separate until stimMapping extraction proceeure is nailed down
%     
%
%
%  Outputs:
...
%
%
%  What to do AFTER running this function: once the pulse updown files are moved to stimMapping, 
%      1) create a set of session_X folders and move the new files in stimMapping to each folder                [now done automatically in nk_split]
%      2) use "saveas" to create a copy of the file containing stimulation pulse timing renamed as session.log, [now done automatically in nk_split]
%      3) then run behavioralProcessing on stimMapping to convert that to an events file for analysis
%             see createStimEvents_v1b() for more info about how to tweak the synthetic session.log file
%
%
...
FORCE_EVENT_REEXTRACTION    = 1 ;  % 0 or 1: re-extract all behavioral events using "behavioralProcessing"; if 0 only sessions missing events will be extracted
...
EXTRACT_REREF_FILES         = 1 ;  % 0 or 1: for stim mapping assume we will only look at noreref, at least initially   STIM_MAPPING SPECIFIC
...
if (length(days)==0)
    ...
    if (MISSING_EXTRACTION)
        ...
        for iList=needsExtr,
            ...
            if (fid~=-1)
                foo                        = textscan(fid,'%s');
                tagNameOrder               = foo{1};
                %check to see if R, EKG, or REF are in the tag list, if so, don't add them
    ...
    if EXTRACT_REREF_FILES==0,
        fprintf('  HEADS UP: one or more rereferencing file missing but NOT created because "EXTRACT_REREF_FILES"==0');
        MISSING_ANY_REREF_FILES=1;  %- just use this to skip over rereferencing...
    end
