function events=extractAttentionEvents(subject,expDir,sessionNum)
% subject = 'Jian'
% sessionNum = 0
% expDir = '/Users/ellenbogenrl/Rachel/data/eeg/NIH011/behavioral/attentionTask/'
sessFile = fullfile(expDir,['session_' num2str(sessionNum)],'session.log');
...
while true
    ...
    switch type
        case 'SESS_START'
            ...
            events(index).session = sessionNum;
        ...
        case 'TASK_FC'
            ...
            annFile = fullfile(expDir,['session_' num2str(sessionNum)],annFileName);
            ...
            if strcmpi(events(index).recognizedWord, events(index).presentedWord)
                ...
                strcmpi(events(index).recognizedWord, events(index).foilWord);
