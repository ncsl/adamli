function events=extractAttentionEvents(subject,session)
%subject = 'NIH003'
%session = 1
expDir = ['/Users/ellenbogenrl/Rachel/data/eeg/' subject '/behavioral/attentionTask' ];

sessFile = fullfile(expDir,['session_' num2str(session)],'session.log');
outputDir = fullfile(expDir,['session_' num2str(session)]);
...
while true
  ...
  switch type
      ...
      case 'TASK_FC'
          ...
          elseif strcmpi(events(index).recognizedWord, events(index).foilWord)
