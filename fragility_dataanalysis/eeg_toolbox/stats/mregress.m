function [Coefficients, S_err, XTXI, R_sq, F_val, Coef_stats, Y_hat, residuals, covariance] = mregress(Y, X, INTCPT,twotailed)

% MREGRESS Performs multiple linear regression analysis of X
% (independent) on Y (dependent).
%
%           Usage:
%
%           [Coefficients, S_err, XTXI, R_sq, F_val, Coef_stats,
%           Y_hat, residuals, covariance] ...  = mregress(Y, X,
%           INTCPT,twotailed)
%
%	    INTCPT = 1; include a y-intercept in the model
%           INTCPT = 0; DO NOT include a y-intercept in the model
%           twotailed = 1; do a two-tailed t-test on each of the
%           coefficients (optional), if it is set to zero, a
%           one-tailed test is done
%           Returns:
%
...
% adapted by Marieke van Vugt (MvV)
%
% Please note that for the case when the intercept of the model equals
% zero, the definition of R-squared and the F-statistic change
% mathematically. For a linear model containing the y-intercept,
% R-squared refers to the amount of variance around the mean of the
% dependent variable (y) which is explained by the variance around the
% mean of the independent variables (x). For a linear model NOT
% containing the y-intercept, R-squared measures the amount of
% variance around ZERO of the dependent variable which is explained by
% the variance around ZERO of the independent variable.  If the same
% equation for R-squared is used for both with and without a
% y-intercept (namely R-squared = [Sum of Squares of the Regression] /
% [ Total sum of the squares]), then R-squared may be a NEGATIVE value
% for some data. For this reason, this subroutine will calculate
% R-squared using the total un-corrected sum of the squares. In
% effect, this approach avoids negative R-squares but may lack any
% meaningful interpretation for the "goodness-of-fit" in the model. It
% has been suggested by some texts that a more useful approach is to
% always use the case where y-intercept is included in the
% model. However, as with all statistical analyses, it is prudent to
% simply be aware of what your data "looks" like and what your
% statistical tools are actually measuring in order to generate a
% useful analysis.
%
...
% MvV: Remove NaN datapoints
goodInd = find(~isnan(Y));
origLength = length(Y);
Y = Y(goodInd);
X = X(goodInd,:);
[n_x, k] = size(X);
...
% $$$ % ====================================
% $$$ % The follwing equation is from Judge G, et al. "An Introduction
% $$$ to the theory % and practice of econometrics", New York : Wiley,
% $$$ 1982. It is the % squared (Pearson) correlation coefficient
% $$$ between the predicted and % dependent variables. It is the same
% $$$ equation regardless of whether an % intercept is included in the
% $$$ model; however, it may yield a negative % R-squared for a
% $$$ particularily bad fit.
...
%  --------------------------------------
    ...
    % we removed the NaN points, now add back the NaNs in the
    % residuals
    oldresiduals = residuals;
    fprintf('check size of residuals')
    residuals = NaN(origLength,1);
    residuals(goodInd) = oldresiduals; 
    clear oldresiduals;
...
%  -----------------------------------------------------------------------------
	...
	p_value = 2 * (1 - tcdf(abs(Coefficients./C), (n - k)));
      ...
      if (INTCPT == 1)
	p_value =  (1 - tcdf(Coefficients./C, (n - (k + 1))));
      else
	p_value =  (1 - tcdf(Coefficients./C, (n - k)));
      end      
    end
