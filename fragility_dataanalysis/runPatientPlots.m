...
for p=1:length(patients)
    ...
    %%- Extract an example
    adjMatDir = fullfile(serverDir, '/adjmats/adaptivefilter/', strcat('win', num2str(winSize), ...
        '_step', num2str(stepSize), '_freq', num2str(frequency_sampling)), patient);
    % directory that computed perturbation structs are saved
    finalDataDir = fullfile(serverDir, 'perturbationmats/adaptivefilter/', strcat('win', num2str(winSize), ...
    ...
    final_data = load(fullfile(finalDataDir, strcat(patient, ...
            '_pertmats_', lower(TYPE_CONNECTIVITY), ...
