...
# 03: Run either connectivity, or both
for patient in $patients; do
	echo $patient
	if [[ "$RUNCONNECTIVITY" -eq 1 ]]; then
		# runAdjMat.sh patient
		sh ./runConnectivity.sh $patient $winSize $stepSize $radius
	else
		echo "Running perturbation computation."
		# run perturbation analysis only
		sh ./runPerturbation.sh $patient $winSize $stepSize $radius
	fi
done

# run perturbation if had to run connectivity first
# if [[ "$RUNCONNECTIVITY" -eq 1 ]]; then
# 	for patient in $patients; do
# 		echo "Running perturbation computation."
# 		# run perturbation analysis only
