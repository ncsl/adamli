...
function generate_slurm(patient, winSize, stepSize, radius, ...
    ...
    %-
    command = sprintf(strcat('sbatch --array=1-%s --time=%s --partition=%s ', ...
                        '--nodes=%s --ntasks-per-node=%s --cpus-per-task=%s ', ...
                        '--jobname=%s run_job.sbatch --export=%s,%s,%s'), ...
                    Nbatch, walltime, partition, numNodes, numTasks, numCPUs, jobname, ...
                    patient, num2str(winSize), num2str(stepSize));
