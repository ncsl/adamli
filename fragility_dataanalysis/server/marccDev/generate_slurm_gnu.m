...
%     else   error('Neither Work nor Home EEG directories exist! Exiting'); end
    ...
    for iPat=1:numPats
        ...
        basecommand = sprintf(strcat(exportcommand, ...
                    ' sbatch --exclusive --time=%s --partition=%s --nodes=%d --ntasks-per-node=%d --cpus-per-task=%d --mem=%d --job-name=%s '), ...
                         num2str(walltime), partition, numNodes, numTasks, numCPUs, memNode, job_name); 
        if strcmp(partition, 'scavenger')
            basecommand = sprintf(strcat(exportcommand, ...
                    ' sbatch --exclusive --time=%s --partition=%s --qos=%s --mem=%d --nodes=%d --ntasks-per-node=%d --cpus-per-task=%d --mem=%d --job-name=%s '), ...
